...
搜"微信"打 `wx`，搜"支付宝"打 `zfb`，搜"百度网盘"打 `bdwp`。不切换输入法，几个字母定位到想要的软件。

**连续片段匹配**：搜索词不要求从开头匹配，只要在名称的拼音或英文中**连续出现**即可命中。比如搜 `xin` 能匹配到"微信"（weixin 中包含 xin），搜 `otos` 能匹配到"Photoshop"（photoshop 中包含 otos）。记不全或打错一两个字母也不影响。

也支持空格分词，精准匹配。比如搜 `wx file` 只匹配名称中同时包含"微信"和"文件"相关词的应用。
